...
describe("Tag Endpoints", () => {
  it("GET /tags should return all system task layout tags", async () => {
    const res = await request(app).get("/tags");
    expect(res.statusCode).toEqual(200);
    expect(Array.isArray(res.body)).toBe(true);
  });

  it("GET /tags/:id should parse item documentation parameters cleanly", async () => {
    const res = await request(app).get(`/tags/${mockObjectId}`);
    expect([200, 404]).toContain(res.statusCode);
  });
});
